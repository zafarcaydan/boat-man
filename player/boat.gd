...
func spawn_feature(new_feature: OceanFeature, offset: float, dist_multiplier: float = 1) -> bool:
	...
	for comparison_feature in get_tree().get_nodes_in_group("Spawnable"):
		...
		if sqrt(relative_pos.x ** 2 + relative_pos.y ** 2) < spawn_dist * 0.9 * new_feature.comparison_dist * comparison_feature.comparison_dist: 
			return false
